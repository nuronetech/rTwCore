...
{
    //Check to see if the zMuBdI is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zAR spend does not have a valid signature", __func__);
    }

    //Is the serial # already in the blockchain
    int nHeightTx = 0;
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zAR spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() && !spend.HasValidSerial(Params().Zerocoin_Params()))
        return error("%s : zAR spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zAR spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip()))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zar");
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
      ...
      int64_t nSubsidy = 0;
      if (nHeight == 0) {
		  nSubsidy = 1 * COIN; //Central Codes
	  } else if (nHeight == 10) {
          nSubsidy = 7000000000 * COIN; //PoS Servers + Pre-Sale
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZMuBdIStake)
{
    int64_t ret = 0;
	...
	// 90% for Masternodes from block 1000
     if (nHeight <= 100) {
		  ...
		  ret = blockValue  / 100 * 90;               // %90
                  ...
                  //When zMuBdI is staked, masternode only gets 2 MuBdI
                  ret = 3 * COIN;
                 if (isZMuBdIStake)
                  ret = 2 * COIN;
     }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from MuBdI
         * addresses should still be handled by the typical mubdi based undo code
         * */
...
{
    RenameThread("mubdi-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMuBdIMinted()
{
...
void RecalculateZMuBdISpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMuBdI supply
...
{
    // MuBdI: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZMuBdISupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!fVerifyingBlocks && pindex->nHeight == Params().Zerocoin_StartHeight() + 1) {
        RecalculateZMuBdIMinted();
        RecalculateZMuBdISpent();
        RecalculateMuBdISupply(1);
    }

   //Track zMuBdI money supply in the block index
    if (!UpdateZMuBdISupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zAriA supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMuBdISpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MuBdI to zMuBdI
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMuBdI spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zAriA serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(CStakeInput* stake)
{
    if (chainActive.Height() + 1 < Params().Zerocoin_Block_V2_Start())
        return error("%s: zAR stake block is less than allowed start height", __func__);
     if (CZMuBdIStake* zMuBdI = dynamic_cast<CZMuBdIStake*>(stake)) {
        CBlockIndex* pindex = zMuBdI->GetIndexFrom();
         if (chainActive.Height() - pindex->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zAR stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zMuBdI->GetValue()));
        if (nChecksum200 != zMuBdI->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zMuBdI->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        CStakeInput* stake = nullptr;
         ...
         if (stake->IsZMuBdI() && !ContextualCheckZerocoinStake(stake))
            return state.DoS(100, error("%s: staked zAR fails context checks", __func__));
         uint256 hash = block.GetHash();
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zAriA mints and %d zAriA spends\n", __func__, nMints, nSpends);
