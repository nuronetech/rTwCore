...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
  	    ...
  	    uint256 txid = tx.GetHash();
		    ...
		    //Give a high priority to zerocoinspends to get into the next block
                    //Priority = (age^6+100000)*amount - gives higher priority to zARs that have been in mempool long
                    //and higher priority to zARs that are large in value
		    ...
		    double nTimePriority = std::pow(GetAdjustedTime() - nTimeSeen, 6);
                    ...
                    // zAR spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zMuBdI spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zMuBdI serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
// ***TODO*** that part changed in mubdi, we are using a mix with old one here for now
    ...
    while (fGenerateMuBdIs || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (IsInitialBlockDownload() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
                ...
                MilliSleep(5000);
                if (!fGenerateMuBdIs && !fProofOfStake)
                    continue;
            }
        ...
        if (fProofOfStake) {
            ...
            //Sign block with the zPIV key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("MuBdIMiner(): Signing new block with zPIV key failed \n");
                    continue;
                }
            } else if (!SignBlock(*pblock, *pwallet)) {
                LogPrintf("MuBdIMiner(): Signing new block with UTXO key failed \n");
                continue;
            }
...
void static ThreadMuBdIMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadMuBdIMiner() exception");
    } catch (...) {
        LogPrintf("ThreadMuBdIMiner() exception");
    }

    LogPrintf("ThreadMuBdIMiner exiting\n");
}
