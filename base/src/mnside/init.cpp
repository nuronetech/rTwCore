...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, aria, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_ARIA_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-backupzaria=<n>", strprintf(_("Enable automatic wallet backups triggered after each zAR minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeariaamount=<n>     " + strprintf(_("Keep N AR anonymized (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2010-%i AriA Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
bool InitSanityCheck(void)
{
    if (!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.aria.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //AriA specific: zerocoin and spork DB's
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZAriABackups = GetBoolArg("-backupzaria", true);
...
//    }
//
//    nAnonymizeAriAAmount = GetArg("-anonymizeariaamount", 0);
...
//    if (nAnonymizeAriAAmount < 2) nAnonymizeAriAAmount = 2;
    ...
    */
    ...
    if (pwalletMain)
        GenerateAriAs(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", 1));
#endif
