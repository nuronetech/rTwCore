...
const QString AriAGUI::DEFAULT_WALLET = "~Default";

AriAGUI::AriAGUI(const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
                                                                            clientModel(0),
                                                                            walletFrame(0),
...
void AriAGUI::createActions(const NetworkStyle* networkStyle)
{
...
void AriAGUI::setClientModel(ClientModel* clientModel)
{
...
#ifdef ENABLE_WALLET
bool AriAGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    if (!walletFrame)
        return false;
...
bool AriAGUI::setCurrentWallet(const QString& name)
{
    if (!walletFrame)
        return false;
...
void AriAGUI::setWalletActionsEnabled(bool enabled)
{
...
void AriAGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void AriAGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AriAGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AriAGUI::showHelpMessageClicked()
{
...
void AriAGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AriAGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AriAGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void AriAGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AriAGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AriAGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AriAGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

void AriAGUI::gotoMultisigCreate()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(0);
}

void AriAGUI::gotoMultisigSpend()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(1);
}

void AriAGUI::gotoMultisigSign()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(2);
}

void AriAGUI::gotoBip38Tool()
{
    if (walletFrame) walletFrame->gotoBip38Tool();
}

void AriAGUI::gotoMultiSendDialog()
{
    multiSendAction->setChecked(true);
    if (walletFrame)
        walletFrame->gotoMultiSendDialog();
}
void AriAGUI::gotoBlockExplorerPage()
{
...
void AriAGUI::setNumConnections(int count)
{
    QString icon;
    switch (count) {
    case 0:
...
void AriAGUI::setNumBlocks(int count)
{
    if (!clientModel)
        return;
    ...
    // Set icon state: spinning if catching up, tick otherwise [masternode enable]
    //    if(secs < 25*60) // 90*60 for aria but we are 4x times faster
...
void AriAGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
void AriAGUI::changeEvent(QEvent* e)
{
...
void AriAGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
#ifdef ENABLE_WALLET
void AriAGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
    ...
    if(!bdisableSystemnotifications){
        ...
        message((amount) < 0 ? (pwalletMain->fMultiSendNotify == true ? tr("Sent MultiSend transaction") : tr("Sent transaction")) : tr("Incoming transaction"),
            tr("Date: %1\n"
               ...
               "Address: %4\n")
                .arg(date)
                .arg(AriAUnits::formatWithUnit(unit, amount, true))
                .arg(type)
                .arg(address),
...
void AriAGUI::dragEnterEvent(QDragEnterEvent* event)
{
    // Accept only URIs
...
void AriAGUI::dropEvent(QDropEvent* event)
{
    if (event->mimeData()->hasUrls()) {
...
bool AriAGUI::eventFilter(QObject* object, QEvent* event)
{
...
bool AriAGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AriAGUI::setEncryptionStatus(int status)
{
    switch (status) {
...
void AriAGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void AriAGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AriAGUI::detectShutdown()
{
...
void AriAGUI::showProgress(const QString& title, int nProgress)
{
    if (nProgress == 0) {
...
static bool ThreadSafeMessageBox(AriAGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void AriAGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void AriAGUI::unsubscribeFromCoreSignals()
{
...
void AriAGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
...
{
    menu = new QMenu();
    foreach (AriAUnits::Unit u, AriAUnits::availableUnits()) {
        QAction* menuAction = new QAction(QString(AriAUnits::name(u)), this);
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits)
{
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        setPixmap(QIcon(":/icons/unit_" + AriAUnits::id(newUnits)).pixmap(39, STATUSBAR_ICONSIZE));
    } else {
        setPixmap(QIcon(":/icons/unit_t" + AriAUnits::id(newUnits)).pixmap(39, STATUSBAR_ICONSIZE));
    }
}
