...
// click on header
    ...
    // see https://github.com/aria/aria/issues/5716
...
{
    GUIUtil::setClipboard(AriAUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = AriAUnits::ARA;
    ...
    l2->setText(AriAUnits::formatWithUnit(nDisplayUnit, nAmount));   // Amount
    l3->setText(AriAUnits::formatWithUnit(nDisplayUnit, nPayFee));   // Fee
    l4->setText(AriAUnits::formatWithUnit(nDisplayUnit, nAfterFee)); // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));   // Bytes
    l6->setText(sPriorityLabel);                                        // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                          // Dust
    l8->setText(AriAUnits::formatWithUnit(nDisplayUnit, nChange));   // Change
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(AriAUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK())) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red, if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(AriAUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK()));

    QString toolTip3 = tr("This label turns red, if any recipient receives an amount smaller than %1.").arg(AriAUnits::formatWithUnit(nDisplayUnit, ::minRelayTxFee.GetFee(546)));
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                sAddress = QString::fromStdString(CAriAAddress(outputAddress).ToString());
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, AriAUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, AriAUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, AriAUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, AriAUnits::format(nDisplayUnit, nSum));
