...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    return CAriAAddress(keyID).ToString();
}


CAriAAddress GetAccountAddress(string strAccount, bool bForceNew = false)
{
    ...
    return CAriAAddress(account.vchPubKey.GetID());
}
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    CAriAAddress address(params[0].get_str());
    if (!address.IsValid())
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (const PAIRTYPE(CAriAAddress, CAddressBookData) & item, pwalletMain->mapAddressBook) {
        const CAriAAddress& address = item.first;
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (set<CTxDestination> grouping, pwalletMain->GetAddressGroupings()) {
        ...
        BOOST_FOREACH (CTxDestination address, grouping) {
            ...
            addressInfo.push_back(CAriAAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CAriAAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CAriAAddress(address).Get())->second.name);
            }
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    // aria address
    CAriAAddress address = CAriAAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AriA address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    string strAccount = AccountFromValue(params[0]);
    CAriAAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AriA address");
    CAmount nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    set<CAriAAddress> setAddress;
    vector<pair<CScript, CAmount> > vecSend;

    CAmount totalAmount = 0;
    vector<string> keys = sendTo.getKeys();
    BOOST_FOREACH(const string& name_, keys) {
        CAriAAddress address(name_);
        if (!address.IsValid())
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    return CAriAAddress(innerID).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CAriAAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH (const PAIRTYPE(CAriAAddress, CAddressBookData) & item, pwalletMain->mapAddressBook) {
        ...
        map<CAriAAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue printAddresses()
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        std::string strAdd = CAriAAddress(utxoAddress).ToString();
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() == 1) {
        ...
        if (strCommand == "print") {
            ...
            if (CAriAAddress(pwalletMain->vMultiSend[0].first).IsValid()) {
    ...
    if (params.size() == 2 && params[0].get_str() == "disable") {
        ...
        if (!CAriAAddress(disAddress).IsValid())
    ...
    string strAddress = params[0].get_str();
    CAriAAddress address(strAddress);
    if (!address.IsValid())
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CAriAAddress address = CAriAAddress(); // Optional sending address. Dummy initialization here.
    if (params.size() == 5) {
        ...
        address = CAriAAddress(params[4].get_str());
        if(!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AriA address");
    }
    ...
    for (unsigned int i = 0; i < wtx.vout.size(); i++) {
        ...
        if(txout.scriptPubKey.IsZerocoinMint())
            ...
            out.push_back(Pair("address", CAriAAddress(dest).ToString()));
        vout.push_back(out);
    }
