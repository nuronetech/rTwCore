...
bool ReadKeyValue(CWallet* pwallet, CDataStream& ssKey, CDataStream& ssValue, CWalletScanState& wss, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name") {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CAriAAddress(strAddress).Get()].name;
        } else if (strType == "purpose") {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CAriAAddress(strAddress).Get()].purpose;
        } else if (strType == "tx") {
            uint256 hash;
            ssKey >> hash;
            CWalletTx wtx;
        ...
        } else if (strType == "multisend") //presstab HyperStake
        {
            ...
            std::pair<std::string, int> pMultiSend;
            ssValue >> pMultiSend;
            if (CAriAAddress(pMultiSend.first).IsValid()) {
        ...
        } else if (strType == "mdisabled") //presstab HyperStake
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CAriAAddress(strAddress).Get(), strKey, strValue)) {
