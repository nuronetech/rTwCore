...
MuBdIGUI::MuBdIGUI(const NetworkStyle* networkStyle, QWidget* parent) : QMainWindow(parent),
                                                                            clientModel(0),
                                                                            walletFrame(0),
...
void MuBdIGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mubdi: URIs)"));
    ...
    privacyAction->setStatusTip(tr("Privacy Actions for zrTw"));
    ...
    openAction->setStatusTip(tr("Open a rtw: URI or payment request"));
...
void MuBdIGUI::setWalletActionsEnabled(bool enabled)
{
...
void MuBdIGUI::showHelpMessageClicked()
{
...
void MuBdIGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MuBdIGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MuBdIGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void MuBdIGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MuBdIGUI::gotoPrivacyPage()
{
    privacyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoPrivacyPage();
}

void MuBdIGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MuBdIGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MuBdIGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

void MuBdIGUI::gotoMultisigCreate()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(0);
}

void MuBdIGUI::gotoMultisigSpend()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(1);
}

void MuBdIGUI::gotoMultisigSign()
{
    if(walletFrame) walletFrame->gotoMultisigDialog(2);
}

void MuBdIGUI::gotoBip38Tool()
{
    if (walletFrame) walletFrame->gotoBip38Tool();
}

void MuBdIGUI::gotoMultiSendDialog()
{
    multiSendAction->setChecked(true);
    if (walletFrame)
        walletFrame->gotoMultiSendDialog();
}
void MuBdIGUI::gotoBlockExplorerPage()
{
...
void MuBdIGUI::setNumConnections(int count)
{
    QString icon;
    switch (count) {
    case 0:
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to MuBdI network", "", count));
}

void MuBdIGUI::setNumBlocks(int count)
{
    if (!clientModel)
        return;
    ...
    // Set icon state: spinning if catching up, tick otherwise [masternode disable]
    //    if(secs < 25*60) // 90*60 for mubdi but we are 4x times faster
    if(secs < 25*60) {
...
void MuBdIGUI::dragEnterEvent(QDragEnterEvent* event)
{
    // Accept only URIs
...
void MuBdIGUI::dropEvent(QDropEvent* event)
{
    if (event->mimeData()->hasUrls()) {
...
bool MuBdIGUI::eventFilter(QObject* object, QEvent* event)
{
...
#ifdef ENABLE_WALLET
void MuBdIGUI::setStakingStatus()
{
    if (pwalletMain)
...
void MuBdIGUI::setAutoMintStatus()
{
    if (fEnableZeromint) {
...
bool MuBdIGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MuBdIGUI::setEncryptionStatus(int status)
{
    switch (status) {
...
void MuBdIGUI::showProgress(const QString& title, int nProgress)
{
    if (nProgress == 0) {
...
{
    menu = new QMenu();
    foreach (MuBdIUnits::Unit u, MuBdIUnits::availableUnits()) {
        QAction* menuAction = new QAction(QString(MuBdIUnits::name(u)), this);
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits)
{
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        setPixmap(QIcon(":/icons/unit_" + MuBdIUnits::id(newUnits)).pixmap(39, STATUSBAR_ICONSIZE));
    } else {
        setPixmap(QIcon(":/icons/unit_t" + MuBdIUnits::id(newUnits)).pixmap(39, STATUSBAR_ICONSIZE));
    }
}
