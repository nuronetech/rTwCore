...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMuBdIAmount"))
        settings.setValue("nAnonymizeMuBdIAmount", 1000);

    nAnonymizeMuBdIAmount = settings.value("nAnonymizeMuBdIAmount").toLongLong();
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMuBdIAmount:
            return QVariant(nAnonymizeMuBdIAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMuBdIAmount:
            nAnonymizeMuBdIAmount = value.toInt();
            settings.setValue("nAnonymizeMuBdIAmount", nAnonymizeMuBdIAmount);
            emit anonymizeMuBdIAmountChanged(nAnonymizeMuBdIAmount);
            break;
        case CoinControlFeatures:
