                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMuBdI ought to be enough for anybody." - Bill Gates, 2017
    ui->zMuBdIpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMuBdISyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMuBdI->setEnabled(false);
        ui->pushButtonMintzMuBdI->setToolTip(tr("zrTw is currently disabled due to maintenance."));

        ui->pushButtonSpendzMuBdI->setEnabled(false);
        ui->pushButtonSpendzMuBdI->setToolTip(tr("zrTw is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMuBdIpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMuBdI_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zrTw is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zMuBdI, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zrTw...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zrTw in ") +
...
void PrivacyDialog::on_pushButtonSpendzMuBdI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zrTw is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMuBdI
        sendzMuBdI();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMuBdI
    sendzMuBdI();
}

void PrivacyDialog::on_pushButtonZMuBdIControl_clicked()
{
	if (!walletModel || !walletModel->getOptionsModel())
        return;
	
    ZMuBdIControlDialog* zMuBdIControl = new ZMuBdIControlDialog(this);
    zMuBdIControl->setModel(walletModel);
    zMuBdIControl->exec();
}

void PrivacyDialog::setZMuBdIControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMuBdISelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMuBdI()
{
    QSettings settings;

    // Handle 'Pay To' address options
    CMuBdIAddress address(ui->payTo->text().toStdString());
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid rTw Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMuBdIpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMuBdI is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " rTw </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zrTw</b>";
    ...
    // use mints from zMuBdI selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMuBdIControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMuBdIControlDialog::GetSelectedMints();
    }
	...
	if (walletModel && walletModel->getAddressTableModel()) {
        // If zMuBdI was spent successfully update the addressbook with the label
    ...
    // Clear zMuBdI selector in case it was used
    ZMuBdIControlDialog::listSelectedMints.clear();
	ui->labelzMuBdISelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zrTw Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " rTw, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zrTw Mint");
        else if(ExtractDestination(txout.scriptPubKey, dest))
            strStats += tr(CMuBdIAddress(dest).ToString().c_str());
        strStats += "\n";
    }
    double fDuration = (double)(GetTimeMillis() - nTime)/1000.0;
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zrTw </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zrTw "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zrTw "));
    ui->labelzMuBdIAmountValue->setText(MuBdIUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, MuBdIUnits::separatorAlways));

    // Display AutoMint status
    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zrTw </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zrTw </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzMuBdISyncStatus->setVisible(fShow);
}
