    ...
    if (wtx.IsCoinStake()) {
        ...
        if (!wtx.IsZerocoinSpend() && !IsMine(*wallet, address)) {
            ...
            if (wtx.IsZerocoinSpend()) {
                ...
                sub.type = TransactionRecord::StakeZMuBdI;
        ...
        for (const CTxOut txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.type = TransactionRecord::ZerocoinSpend_Change_zMuBdI;
                sub.address = mapValue["zerocoinmint"];
            ...
            if (ExtractDestination(txout.scriptPubKey, address))
                strAddress = CMuBdIAddress(address).ToString();
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || TransactionRecord::StakeZMuBdI || type == TransactionRecord::MNReward) {
