 ...
 */
    ...
    typeWidget->addItem(tr("Minted"), TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) | TransactionFilterProxy::TYPE(TransactionRecord::StakeZMuBdI));
    typeWidget->addItem(tr("Masternode Reward"), TransactionFilterProxy::TYPE(TransactionRecord::MNReward));
    typeWidget->addItem(tr("Received rTw from zrTw"), TransactionFilterProxy::TYPE(TransactionRecord::RecvFromZerocoinSpend));
    ...
    typeWidget->addItem(tr("Zerocoin Spend, Change in zrTw"), TransactionFilterProxy::TYPE(TransactionRecord::ZerocoinSpend_Change_zMuBdI));
...
void TransactionView::changedAmount(const QString& amount)
{
    ...
    if (model) {
        // Replace "," by "." so MuBdIUnits::parse will not fail for users entering "," as decimal separator
        QString newAmount = amount;
        newAmount.replace(QString(","), QString("."));

        if (MuBdIUnits::parse(model->getOptionsModel()->getDisplayUnit(), newAmount, &amount_parsed)) {
...
void TransactionView::exportClicked()
{
    ...
    if (model) {
        ...
        writer.addColumn(MuBdIUnits::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
...
void TransactionView::computeSum()
{
    ...
    QString strAmount(MuBdIUnits::formatWithUnit(nDisplayUnit, amount, true, MuBdIUnits::separatorAlways));
