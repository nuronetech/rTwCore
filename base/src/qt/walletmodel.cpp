...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            CScript scriptPubKey = GetScriptForDestination(CMuBdIAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 rTw.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
{
    QString strAddress = QString::fromStdString(CMuBdIAddress(address).ToString());
...
static void NotifyzMuBdIReset(WalletModel* walletmodel)
{
...
void WalletModel::subscribeToCoreSignals()
{
	...
	wallet->NotifyzMuBdIReset.connect(boost::bind(NotifyzMuBdIReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
	...
	wallet->NotifyzMuBdIReset.disconnect(boost::bind(NotifyzMuBdIReset, this));
}
