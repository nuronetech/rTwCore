...
UniValue obfuscation(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <mubdiaddress> <amount>\n"
            "mubdiaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <mubdiaddress> <amount>\n"
            "mubdiaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
    ...
    BOOST_FOREACH (PAIRTYPE(int, CMasternode) & s, vMasternodeRanks) {
        ...
        if (mn != NULL) {
            if (strFilter != "" && strTxHash.find(strFilter) == string::npos &&
                ...
                CMuBdIAddress(mn->pubKeyCollateralAddress.GetID()).ToString().find(strFilter) == string::npos) continue;
...
UniValue masternodeconnect(const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.1.3:43088\"") + HelpExampleRpc("masternodeconnect", "\"192.168.1.3:43088\""));
