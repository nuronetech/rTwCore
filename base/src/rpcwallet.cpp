...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"mubdiaddress\"    (string) The new aria address\n"
            "\nExamples:\n" +
...
CMuBdIAddress GetAccountAddress(string strAccount, bool bForceNew = false)
{
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"mubdiaddress\"   (string) The account aria address\n"
            "\nExamples:\n" +
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"mubdiaddress\"  (string, required) The aria address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"mubdiaddress\"  (string, required) The aria address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"mubdiaddress\"  (string) a aria address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
    ...
    BOOST_FOREACH (const PAIRTYPE(CMuBdIAddress, CAddressBookData) & item, pwalletMain->mapAddressBook) {
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"mubdiaddress\"  (string, required) The aria address to send to.\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"mubdiaddress\",     (string) The aria address\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            HelpRequiringPassphrase() + "\n"
                                        "\nArguments:\n"
                                        "1. \"mubdiaddress\"  (string, required) The aria address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"mubdiaddress\"  (string, required) The aria address for transactions.\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tomubdiaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a aria address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"
                                        "\nArguments:\n"
                                        "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
                                        "2. \"tomubdiaddress\"  (string, required) The aria address to send funds to.\n"
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                     ...
                     "\nResult:\n"
                     "\"mubdiaddress\"  (string) A aria address associated with the keys.\n"
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CMuBdIAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH (const PAIRTYPE(CMuBdIAddress, CAddressBookData) & item, pwalletMain->mapAddressBook) {
        ...
        map<CMuBdIAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"mubdiaddress\",    (string) The aria address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"mubdiaddress\",    (string) The aria address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"mubdiaddress\",   (string) The aria address involved in the transaction\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending AriAs\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending AriAs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in AriA/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total AriA balance of the wallet\n"
    ...
    if (params.size() == 1) {
        ...
        if (strCommand == "print") {
            ...
            if (CMuBdIAddress(pwalletMain->vMultiSend[0].first).IsValid()) {
                ...
                if (!walletdb.WriteMSettings(true, false/*, pwalletMain->fMultiSendMasternodeReward*/, pwalletMain->nLastMultiSendHeight)) {
                    UniValue obj(UniValue::VOBJ);
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "amount: (numeric, required) Enter an amount of AR to convert to zAR\n"
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zAriA is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zAriA (zerocoins) into AriA. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over AriA (change), the wallet can convert it automatically back to zerocoins [true]\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zMuBdI
    bool fMinimizeChange = params[2].get_bool();    // Minimize change
    int nSecurityLevel = params[3].get_int();       // Security level

    CMuBdIAddress address = CMuBdIAddress(); // Optional sending address. Dummy initialization here.
    if (params.size() == 5) {
        ...
        address = CMuBdIAddress(params[4].get_str());
        if(!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AriA address");
    }
    ...
    for (unsigned int i = 0; i < wtx.vout.size(); i++) {
        ...
        if(txout.scriptPubKey.IsZerocoinMint())
            ...
            out.push_back(Pair("address", CMuBdIAddress(dest).ToString()));
        vout.push_back(out);
    }
    ...
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
            "exportzerocoins include_spent ( denomination )\n"
                ...
                "2. \"denomination\"         (integer, optional) Export a specific denomination of zAriA\n"
    ...
    if(fHelp || params.size() == 0)
        throw runtime_error(
            "importzerocoins importdata \n"
                ...
                "\"value\"            (string) the total zAriA value of zerocoin mints that were added\n"
...
UniValue reconsiderzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "reconsiderzerocoins\n"
                "\nCheck archived zAriA list to see if any mints were added to the blockchain.\n"
