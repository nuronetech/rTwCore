...
map<CMuBdIAddress, vector<COutput> > CWallet::AvailableCoinsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    vector<COutput> vCoins;
    AvailableCoins(vCoins, fConfirmed);

    map<CMuBdIAddress, vector<COutput> > mapCoins;
    BOOST_FOREACH (COutput out, vCoins) {
        ...
        mapCoins[CMuBdIAddress(address)].push_back(out);
    }
...
bool CWallet::SelectStakeCoins(std::list<CStakeInput*>& listInputs, CAmount nTargetAmount) const
{
	...
	CMuBdIStake* input = new CMuBdIStake();
        ...
        list<CZerocoinMint> listMints = walletdb.ListMintedCoins(true, true, true);
         for (CZerocoinMint mint : listMints) {
            ...
            if (mint.GetHeight() < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                CZMuBdIStake *input = new CZMuBdIStake(mint);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 MuBdI
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 MuBdI over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 MuBdI+1 ( bit on if present )
            // bit 1 - 1000 MuBdI+1
            // bit 2 - 100 MuBdI+1
            // bit 3 - 10 MuBdI+1
            // bit 4 - 1 MuBdI+1
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 AriA.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 AriA.");
                    } else {
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    for (CStakeInput* stakeInput : listInputs) {
	    ...
	    CAmount nMinFee = 0;
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZMuBdI());
            ...
            //Mark mints as spent
            if (stakeInput->IsZMuBdI()) {
                CZMuBdIStake* z = (CZMuBdIStake*)stakeInput;
                if (!z->MarkSpent(this))
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CMuBdIAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CMuBdIAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if (fFileBacked) {
            ...
            std::string strAddress = CMuBdIAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CMuBdIAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CMuBdIAddress(address).ToString());
}
...
void CWallet::AutoZeromint()
{
    ...
    // zMuBdI are integers > 0, so we can't mint 10% of 9 MuBdI
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zAriA\n", nBalance);
        return;
    }

    // Percentage of zMuBdI we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zAriA (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zMuBdI amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zMuBdI amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zMuBdI We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough MuBdI to mint this denomination
    ...
    CWalletTx wtx;
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zAriA. Current percentage of zAriA: %lf%%\n",
...
void CWallet::AutoCombineDust()
{
    ...
    map<CMuBdIAddress, vector<COutput> > mapCoinsByAddress = AvailableCoinsByAddress(true, nAutoCombineThreshold * COIN);

    //coins are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (map<CMuBdIAddress, vector<COutput> >::iterator it = mapCoinsByAddress.begin(); it != mapCoinsByAddress.end(); it++) {
...
bool CWallet::CreateZMuBdIOutPut(libzerocoin::CoinDenomination denomination, libzerocoin::PrivateCoin& coin, CTxOut& outMint)
{
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZMuBdIOutPut(denomination, coin, outMint)) {
            strFailReason = strprintf("%s: failed to create new zar output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to MuBdI
    ...
    // Sign if these are mubdi outputs - NOTE that zMuBdI outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZMUBDI_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZMUBDI_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason, pindexCheckpoint)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZMUBDI_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    if (nVersion >= 2) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zAR privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZMUBDI_INVALID_WITNESS);
            return false;
        }
        ...
        //Add the coin spend into a MuBdI transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZMUBDI_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus(_("The transaction did not verify"), ZMUBDI_BAD_SERIALIZATION);
            return false;
        }
        ...
        for (const CBigNum& item : listCoinSpendSerial) {
            if (spend.getCoinSerialNumber() == item) {
                //Tried to spend an already spent zMuBdI
                ...
                receipt.SetStatus(_("The coin spend has been used"), ZMUBDI_SPENT_USED_ZMUBDI);
                return false;
            }
        }
        ...
        receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZMUBDI_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus(_("Spend Valid"), ZMUBDI_SPEND_OKAY); // Everything okay

    return true;
}

bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CMuBdIAddress* address)
{
    // Check available funds
    int nStatus = ZMUBDI_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the the smallest available denomination (= 1) of zAriA"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZMUBDI_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zMuBdI transaction
    if (vSelectedMints.empty()) {
        ...
        // Select the zMuBdI mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
	    ...
	    if (nChange > 0 && !address) {
            ...
            //add output to mubdi address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZMUBDI_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZMUBDI_SPEND_OKAY); // Everything okay
...
void CWallet::ZMuBdIBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozmubdibackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozmubdibackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozmubdibackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CMuBdIAddress* addressTo)
{
    // Default: assume something goes wrong. Depending on the problem this gets more specific below
    int nStatus = ZMUBDI_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZMUBDI_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZMUBDI_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& mint : vNewMints) {
            if (!walletdb.EraseZerocoinMint(mint)) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZMUBDI_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    receipt.SetStatus("Spend Successful", ZMUBDI_SPEND_OKAY);  // When we reach this point spending zMuBdI was successful
